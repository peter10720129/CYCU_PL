...
void GetToken( string & token, Type & type, float & tokenVal, vector<string> & command, string & error ) {
  ...
  while ( command[gtokenPtr] == "NEXT_L" )
   gtokenPtr++ ;
  cout << gtokenPtr << ' ' << command.size() << endl ;
...
float Recursive_decent_parser( vector<string> & command, string & error, int & bExp ) {
// < Command > :: = < Statement> ';' | < BooleanExp> ';' | < ArithExp> ';' | QUIT
  float res = 0.0 ;
  int val = -1 ;
  if ( command[0] == "quit" ) {
  ...
  if ( error.size() != 0 ) return -1 ;
  gtokenPtr = 0 ;
  if ( IsArithExp( res, command, error ) ) return res ;
...
int Issametoken( string a, char b ) {
  ...
  else return ! ( Isarith( a[0] ) || Isop( a[0] ) || Isarith( b ) || Isop( b ) ) ;
} // Issametoken()
...
int main() {  
  ...
  while ( ! gquit ) {
    ...
    for ( int i = gtokenPtr ; i < command.size(); i ++ ) 
      ...
      if ( error.size() == 0 ) {
        float res = Recursive_decent_parser( command, error, bExp )  ;
      ...
      else cout << "Unrecognized token with first char : '" << error << '\'' << endl ;  // lexical error 
      
      error = "" ;

    } // while 
    if ( ! gquit ) Lexical_analysis( command, error ) ;
    gtokenPtr = 0 ;
  } // while 
